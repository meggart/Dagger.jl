...
function eager_submit_internal!(ctx, state, task, tid, payload; uid_to_tid=Dict{UInt64,Int}())
    ...
    _args = Base.mapany(args) do pos_arg
        if pos_arg[2] isa EagerThunk
            tid = uid_to_tid[pos_arg[2].uid]
            return pos_arg[1] => state.thunk_dict[tid]
        elseif pos_arg[2] isa Sch.ThunkID
            tid = pos_arg[2].id
            return pos_arg[1] => state.thunk_dict[tid]
        else
    ...
    syncdeps = get(Set, options, :syncdeps)
    _syncdeps = lock(Sch.EAGER_ID_MAP) do id_map
        Base.mapany(syncdeps) do dep
    ...
    options = merge(options, (;syncdeps=_syncdeps))
...
function eager_submit!(ntasks, uid, future, finalizer_ref, f, args, options)
    ...
    elseif myid() != 1
        return remotecall_fetch(eager_submit!, 1, ntasks, uid, future, finalizer_ref, f, args, options, true)
    else
        ...
        return lock(state.lock) do
            eager_submit_internal!(ctx, state, task, tid,
                                   (ntasks, uid, future, finalizer_ref,
                                    f, args, options,
                                    true))
        end
    end
